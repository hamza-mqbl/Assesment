...
router.post(
  ...
  catchAsyncErrors(async (req, res, next) => {
    try {
      ...
      if (!magazine.plans.length)
        return res
          .status(400)
          .json({ error: "No plans available for this magazine" });

      // Calculate total price after discounts
      const discountPrice = magazine.plans.reduce(
        (total, plan) => total + magazine.base_price * (1 - plan.discount),
        0
      );

      // Set renewal date (based on the longest renewal period among plans)
      const maxRenewalPeriod = Math.max(
        ...magazine.plans.map((plan) => plan.renewalPeriod)
      );
  ...
  catchAsyncErrors(async (req, res, next) => {
    try {
      ...
      const formattedSubscriptions = subscriptions.map((sub) => {
        const { _id, user_id, price, renewal_date, is_active, __v } =
          sub.toObject();
        return {
          _id,
          user_id,
          magazine: sub.magazine_id.toObject(),
          price,
          renewal_date,
          is_active,
          __v,
  ...
  catchAsyncErrors(async (req, res, next) => {
    try {
      const subscription = await Subscription.findByIdAndUpdate(
        ...
        req.body,
        { new: true }
      ).populate("magazine_id");
      if (!subscription)
        return res.status(404).json({ error: "Subscription not found" });
      res.json({
        ...subscription.toObject(),
        magazine: subscription.magazine_id.toObject(),
      });
    } catch (err) {
      res.status(400).json({ error: err.message });
    }
